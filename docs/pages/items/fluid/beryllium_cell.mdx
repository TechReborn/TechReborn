![Beryllium Cell](/mods/techreborn/beryllium_cell.png)
=======Beryllium Cell======= The **Beryllium Cell** is a fluid cell
added by TechReborn. You can get by using empty cell on Beryllium fluid
block or from several machine recipes.

##### Recipe

`<mcitem>`{=html}techreborn:industrial_electrolyzer`</mcitem>`{=html}
recipe: \\\\
29x`<mcitem>`{=html}techreborn:emerald_dust`</mcitem>`{=html} +
18x`<mcitem>`{=html}techreborn:empty_cell`</mcitem>`{=html} -\>
2x`<mcitem>`{=html}techreborn:aluminium_dust`</mcitem>`{=html} +
3x`<mcitem>`{=html}techreborn:beryllium_cell`</mcitem>`{=html} +
6x`<mcitem>`{=html}techreborn:silicon_cell`</mcitem>`{=html} +
9x`<mcitem>`{=html}techreborn:compressed_air_cell`</mcitem>`{=html} \\\\
16x`<mcitem>`{=html}techreborn:ender_pearl_dust`</mcitem>`{=html} +
16x`<mcitem>`{=html}techreborn:empty_cell`</mcitem>`{=html} -\>
5x`<mcitem>`{=html}techreborn:nitrogen_cell`</mcitem>`{=html} +
`<mcitem>`{=html}techreborn:beryllium_cell`</mcitem>`{=html} +
4x`<mcitem>`{=html}techreborn:potassium_cell`</mcitem>`{=html} +
6x`<mcitem>`{=html}techreborn:chlorite_cell`</mcitem>`{=html} \\\\

`<mcitem>`{=html}techreborn:fluid_replicator`</mcitem>`{=html} recipe:
\\\\ 10x`<mcitem>`{=html}techreborn:UU_matter`</mcitem>`{=html} -\>
`<mcitem>`{=html}techreborn:beryllium_cell`</mcitem>`{=html}

##### Usage

-   -   Beryllium Cell** used in the following crafting recipe:

`<recipe>`{=html} input air techreborn:neutron_reflector air input
techreborn:neutron_reflector techreborn:beryllium_cell
techreborn:neutron_reflector input air techreborn:neutron_reflector air
output techreborn:thick_neutron_reflector `</recipe>`{=html}

\\\\

-   -   Beryllium Cell** used in the following
        `<mcitem>`{=html}techreborn:fusion_reactor`</mcitem>`{=html}
        recipe: \\\\

`<mcitem>`{=html}techreborn:beryllium_cell`</mcitem>`{=html} +
`<mcitem>`{=html}techreborn:wolframium_cell`</mcitem>`{=html} -\>
`<mcitem>`{=html}techreborn:platinum_dust`</mcitem>`{=html}

##### History

` * Updated for 1.16 version`
